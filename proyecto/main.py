...
# mydb=mysql.connector.connect(host="db-eia.cdjrgcx6ccpf.us-east-1.rds.amazonaws.com",
#                              user="admin",
#                              password="admineia",
#                              database="DB-EIA")
...
async def read_item(item_id):
    try:
      mydb=mysql.connector.connect(host="db-eia.cdjrgcx6ccpf.us-east-1.rds.amazonaws.com",
                             ...
                             database="DB-EIA")
      mycursor=mydb.cursor(buffered=True,dictionary=True) 
      mycursor.execute("SELECT * FROM `DB-EIA`.test_api_dots WHERE devices_id=1")
      # myresult=mycursor.fetchall()
      content={}
      list_dots=mycursor.fetchall()
      mycursor.execute("SELECT * FROM `DB-EIA`.test_api_dots WHERE devices_id=2")
...
async def create_machine():
    try:
      mydb=mysql.connector.connect(host="db-eia.cdjrgcx6ccpf.us-east-1.rds.amazonaws.com",
                             ...
                             database="DB-EIA")
      mycursor=mydb.cursor(buffered=True,dictionary=True)
      mycursor.execute("SELECT * FROM `DB-EIA`.test_api_machines")
      myresult=mycursor.fetchall()
...
async def create_current_dot(dot:Dot_current):
    try:
      mydb=mysql.connector.connect(host="db-eia.cdjrgcx6ccpf.us-east-1.rds.amazonaws.com",
                             ...
                             database="DB-EIA")
      mycursor=mydb.cursor(buffered=True,dictionary=True)
      mycursor.execute("INSERT INTO `DB-EIA`.test_api_dots (value,units,devices_id) VALUES(%s,%s,%s)", (dot.value,dot.units,dot.devices_id))
      mydb.commit()
      return{"dot":dot}
...
async def create_device(devices:Device):
    try:
      mydb=mysql.connector.connect(host="db-eia.cdjrgcx6ccpf.us-east-1.rds.amazonaws.com",
                             ...
                             database="DB-EIA")
      mycursor=mydb.cursor(buffered=True,dictionary=True)
      mycursor.execute("INSERT INTO `DB-EIA`.test_api_devices (divice_name,secret,machines_id) VALUES(%s,%s,%s)", (devices.name,devices.secret,devices.machines_id))
      mydb.commit()
